...
const LANGS = {
  en: {
    ...
    intro:'Your date of birth holds a unique numerical blueprint that Pythagorean numerology has studied for over 2,500 years. By calculating your Life Path Number — derived by summing all digits of your birth date — this free generator identifies the numbers that carry the strongest vibrational resonance with your personal frequency. These lucky numbers are selected using a harmony-weighted algorithm in the Powerball and Mega Millions pool. Optionally add a draw date to unlock three advanced features: a 0–100% compatibility score between your birth energy and the draw-date energy, a per-number energy heatmap (⭐ best match / 🌱 birth energy / 💧 draw energy), and automatic Top 3 best upcoming draw date recommendations. Supports Powerball, Mega Millions, Pick 4, and Pick 3. Multiple sets (1, 3, 5, or 10) can be generated at once. Your data never leaves your browser — 100% free, no registration required.',
  ...
  ja: {
    ...
    start:'ラッキーナンバーを引く',
    intro:'あなたの生年月日は、1,200年以上の歴史を持つ九星気学が解き明かす独自の数字の設計図を持っています。生まれ年から本命星（1〜9の星）を算出し、その五行属性（木・火・土・金・水）に基づく吉数を調和加重アルゴリズムで優先的に選択します。抽選日を入力すると、本命星と日星エネルギーの相性度（0〜100%）・数字ごとのエネルギー内訳（⭐最高相性/🌱生年/💧抽選日）・おすすめ抽選日 TOP 3 が自動表示されます。登録不要・データ保存なし・完全無料でご利用いただけます。',
  ...
  fr: {
    ...
    intro:'Votre date de naissance renferme un schéma numérique unique que la numérologie pythagoricienne décode depuis plus de 2 500 ans. En calculant votre Nombre de Chemin de Vie — la somme réduite de tous les chiffres de votre date de naissance — ce générateur gratuit identifie les numéros portant la plus forte résonance vibratoire avec votre fréquence personnelle. Ces numéros sont sélectionnés par un algorithme à pondération harmonique dans le pool EuroMillions, Loto et EuroJackpot. Ajoutez la date du tirage pour obtenir en plus : un score de compatibilité (0–100%), une carte thermique d\'énergie par numéro (⭐ meilleure correspondance / 🌱 énergie natale / 💧 énergie du tirage) et le Top 3 des prochaines dates de tirage idéales. Aucune inscription, aucune donnée conservée.',
  ...
  es: {
    ...
    intro:'Tu fecha de nacimiento contiene un plano numérico único que la numerología pitagórica ha descifrado durante más de 2.500 años. Calculando tu Número del Camino de Vida — la suma reducida de todos los dígitos de tu fecha de nacimiento — este generador gratuito identifica los números con mayor resonancia vibratoria con tu frecuencia personal. Estos números se seleccionan con un algoritmo de ponderación armónica en el pool de EuroMillions, La Primitiva y BonoLoto. Añadiendo la fecha del sorteo obtienes además: una puntuación de compatibilidad (0–100%), una visualización de energía por número (⭐ mejor coincidencia / 🌱 energía natal / 💧 energía del sorteo) y el Top 3 de las próximas fechas de sorteo más favorables. Sin registro, sin datos guardados.',
  ...
  pt: {
    ...
    intro:'Sua data de nascimento contém um plano numérico único que a numerologia pitagórica decifra há mais de 2.500 anos. Calculando seu Número do Caminho de Vida — a soma reduzida de todos os dígitos da sua data de nascimento — este gerador gratuito identifica os números com maior ressonância vibratória com sua frequência pessoal. Esses números são selecionados com algoritmo de ponderação harmônica no pool da Mega-Sena, Lotofácil e Quina. Ao inserir a data do sorteio, você recebe também: uma pontuação de compatibilidade (0–100%), um mapa de energia por número (⭐ melhor correspondência / 🌱 energia natal / 💧 energia do sorteio) e o Top 3 das próximas datas de sorteio mais favoráveis. Sem cadastro, sem dados armazenados.',
  ...
  it: {
    ...
    intro:'La tua data di nascita racchiude un progetto numerico unico che la numerologia pitagorica decifra da oltre 2.500 anni. Calcolando il tuo Numero del Percorso di Vita — la somma ridotta di tutte le cifre della tua data di nascita — questo generatore gratuito identifica i numeri con la più forte risonanza vibratoria con la tua frequenza personale. Questi numeri vengono selezionati con un algoritmo a ponderazione armonica nel pool di SuperEnalotto, EuroMillions ed EuroJackpot. Aggiungendo la data dell\'estrazione ottieni anche: un punteggio di compatibilità (0–100%), una heatmap energetica per numero (⭐ corrispondenza ottimale / 🌱 energia natale / 💧 energia estrazione) e il Top 3 delle prossime date di estrazione più favorevoli. Nessuna registrazione, nessun dato conservato.',
  ...
  id: {
    ...
    intro:'Tanggal lahir Anda menentukan Weton dalam kalender Jawa — kombinasi hari Masehi 7 hari dengan siklus Pasaran 5 hari (Legi, Pahing, Pon, Wage, Kliwon). Tradisi Primbon yang berusia lebih dari 600 tahun menetapkan bahwa setiap Pasaran memiliki angka-angka hoki yang beresonansi kuat. Generator ini menghitung Pasaran kelahiran Anda dan memilih angka hoki dengan algoritmo bobot harmonis dalam pool Togel 4D. Tambahkan tanggal undian untuk mendapatkan: skor kompatibilitas Weton (0–100%), visualisasi energi per angka (⭐ kecocokan terbaik / 🌱 energi lahir / 💧 energi undian), dan rekomendasi Top 3 tanggal undian terbaik dari 90 hari ke depan. Gratis, tanpa daftar, tanpa penyimpanan data.',
  ...
  ko: {
    ...
    start:'내 행운 번호 뽑기',
    intro:'생년월일에는 사주팔자가 수천 년간 연구해온 고유한 오행 에너지가 담겨 있습니다. 태어난 연도의 천간(甲乙丙丁戊己庚辛壬癸)으로 오행(木火土金水)을 결정하고, 각 오행에 전통적으로 연관된 행운 번호를 조화 가중 알고리즘으로 반영해 로또 6/45 번호를 생성합니다. 목(木)→3·8계열, 화(火)→2·7계열, 토(土)→5·10계열, 금(金)→4·9계열, 수(水)→1·6계열의 전통 오행 수리를 활용합니다. 추첨일을 함께 입력하면 일간 오행과 생년 오행의 상생·상극 관계를 분석해 0~100% 호환도 점수(동기=100%, 상생=85%, 수생=75%, 상극=42%, 피극=38%)를 게이지로 표시하고, 번호별 에너지 구성(⭐최고호환/🌱생년/💧추첨일)과 최적 추첨일 TOP 3를 자동으로 추천합니다. 1~10세트를 한 번에 생성 가능. 데이터는 브라우저에서만 처리되며 저장되지 않습니다.',
